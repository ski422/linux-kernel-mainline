					  ...
					  u64 from, u64 to)
{
	...
	/*
	 * Coalesce the @iteration would-be samples into one record whose
	 * period equals the full window. Same path for -F and -c:
	 * perf_swevent_init_hrtimer() (in event_init) statically maps
	 * attr.freq=1 to a fixed sample_period, so attr.freq is always 0
	 * here. PERF_SAMPLE_PERIOD is enforced at init time, so the scaled
	 * period reaches userspace verbatim; perf's hist accounting yields
	 * identical overhead %, only the "Samples:" column now counts
	 * windows. u64 multiplication avoids overflow at realistic scales.
	 */
	perf_sample_data_init(&data, 0, (u64)period * (u64)iteration);
...
static int task_clock_plus_event_init(struct perf_event *event)
{
	...
	/*
	 * Coalesce path emits a single record with period scaled to the
	 * full off-CPU window. Require PERF_SAMPLE_PERIOD so the scaled
	 * value reaches userspace; without it consumers would fall back
	 * to attr.sample_period and undercount the window by N. Fail
	 * loudly (-EINVAL) rather than silently flipping the bit, so the
	 * ABI stays honest for non-perf openers (libpfm, bpf). perf(1)
	 * sets the bit by default for this event.
	 */
