...
/*
 * Off-CPU sample callchains start deep inside the scheduler (the kernel
 * snapshots regs in task_clock_plus_event_inject() right above
 * finish_task_switch -> __schedule -> blocking primitive). Drop that
 * perf-internal + scheduler prefix so the new leaf is the user-meaningful
 * blocking site (submit_bio, futex_wait, ...).
 *
 *   IOWAIT -> trim up to and including io_schedule (the I/O wrapper that
 *             calls schedule() internally; cutting at io_schedule -- not
 *             at the inner schedule -- surfaces submit_bio as the leaf).
 *   others -> trim up to and including schedule.
 */
...
static bool offcpu_trim_is_boundary(const struct symbol *sym, const char *name)
{
	return sym && !strcmp(sym->name, name);
}

/*
 * Symbol attribution is unaffected by using the unadjusted return address
 * as the new leaf, but srcline / inline / address callchain keys want the
 * byte to land inside the originating call instruction. Apply the -1 IP
 * adjustment only when the user has opted into one of those line-precise
 * display modes.
 */
 ...
 * Returns the new-leaf node when trimming actually fired, NULL otherwise.
 *
 * Cursor frame order:
 *   ORDER_CALLEE -- leaf first; the boundary appears closer to head.
 *   ORDER_CALLER -- root first; the boundary appears closer to tail.
 *
 * If no boundary is found (callchain too short, exclude_callchain_kernel
 * stripped the kernel half, etc.) the cursor is left untouched -- a
 * partial chain is more useful than a wrong one.
 */
...
cut_callee:
		if (!node->next)
			return NULL;
		cursor->first = node->next;
		cursor->nr -= dropped;
		offcpu_trim_promote_leaf(cursor->first);
		return cursor->first;
	}

	/* ORDER_CALLER: remember the boundary closest to the tail. */
	boundary = NULL;
			      ...
			      int max_stack)
{
	...
	/*
	 * sample->ip for an off-CPU sample points inside perf's own inject
	 * frame; once trimming has installed the blocking site as the new
